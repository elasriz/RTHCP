...
class Actor(nn.Module):
    ...
    def forward(self, state):
        #print(state.dtype, self.fc1.weight.dtype, self.fc1.bias.dtype)
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim=[30, 30]):
        ...
        self.fc2 = nn.Linear(hidden_dim[0], hidden_dim[1])
        self.fc4 = nn.Linear(hidden_dim[1], 1)
        for l in [self.fc1, self.fc2, self.fc4]:
            torch.nn.init.orthogonal_(l.weight)  # use a principled initialization
...
# Algorithme TD3
class TD3:
    def __init__(self, state_dim, action_dim, actor_hidden_dim=[400,300], critic_hidden_dim=[400,300], lr_actor=1e-3, lr_critic=1e-3, gamma=0.99, tau=0.005, policy_noise=0.2, noise_clip=0.5, batch_size=256, buffer_capacity=1000000):
    ...
    def load_replay_buffer(self, filename):
        ...
        with open(filename + "_replay_buffer.pkl", "rb") as f:
